...
const { AICheckBlog } = require("../coachController/coachBlogManagement")
const axios = require("axios");

// exerciseController.js
exports.checkAIImage = async (req, res) => {
  try {
    ...
    const response = await axios.get('https://api.sightengine.com/1.0/check.json', {
      params: {
        ...
        'models': 'nudity-2.1',
        'api_user': '1164043222',
        'api_secret': 'PZLqNzrhNfERB68xV4ffbi66aA6D6WAA',
      }
    });

    const nudity = response.data.nudity;

    // Kiểm tra nếu hình ảnh không phù hợp
    const isInappropriate =
      nudity.sexual_activity > 0.3 ||
      nudity.sexual_display > 0.3 ||
      nudity.erotica > 0.3 ||
      nudity.very_suggestive > 0.3;

    return res.status(200).json({
      success: true,
      isInappropriate,
      message: isInappropriate ? false : true
    });

  } catch (error) {
    console.error('Lỗi kiểm tra hình ảnh AI:', error);
    return res.status(500).json({
      success: false,
      message: "Lỗi kiểm tra nội dung hình ảnh",
      error: error.response?.data || error.message
    });
  }
};

exports.checkAIVideo = async (req, res) => {
  try {
    ...
    formData.append('api_user', '1164043222');
    formData.append('api_secret', 'PZLqNzrhNfERB68xV4ffbi66aA6D6WAA');
    ...
    // Kiểm tra nếu video chứa nội dung không phù hợp
    const frames = response.data.frames || [];
    const hasInappropriateContent = frames.some(frame => {
    ...
    return res.status(200).json({
      success: true,
      isInappropriate: hasInappropriateContent,
    ...
    console.error('Lỗi kiểm tra video AI:', error);
    return res.status(500).json({
      success: false,
      message: "Lỗi kiểm tra nội dung video",
      error: error.response?.data || error.message
    });
  }
};
